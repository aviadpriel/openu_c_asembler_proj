    ...
    *ic=INIT_IC;
    ...
    while(fgets( line, LINE_LENGTH,fp))
    {
      ...
      if(!isComment(buff)||*buff=='\n')
      {
        ...
        {
            if(decodeAction(functionIndex,lineCounter,&binWordHead)==ERROR)
                errorFlag=ON;                           
        }
        lineCounter++;
        memset(line,'\0',LINE_LENGTH);        
...
*6 learn to infi 2
*/
int decodeAction(char * buff,int functionIndex,int line,binWordList **binWordHead)
{   
...
int identification(char * buff,int functionIndex,int line,int *ic,SWITCHER secondRun)
{
binWord word;
...
}else if(_action[functionIndex].operends==1)
{
    ...
    if(rval==VALID)
    {
        ...
        {
            binWord labelWord;
            labelWord.opcode=labelWord.orgin=labelWord.dest=labelWord.era=0;  
            int address;
            if((address=isInTheList(buff,labelsHead))==ERROR)
                return ERROR;
...
} else if(_action[functionIndex].operends==2)
    {
        ...
        if(secondRun==ON)
        {
            setBits(functionIndex,"opcode",&word);                
        }        
        firstOp=EMPTY;
        secondOp=EMPTY;
 ...
 int isInTheList(char *label,labelsList **labelsHead,line)
 {
...
{
    word->opcode=labelWord->orgin=labelWord->dest=labelWord->era=0;      
}